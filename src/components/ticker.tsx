          ...
          {[0, 1].map(i => (
            <span key={i} className="inline-block">
              {prices.map((p, j) => {
                ...
                return (
                  <span key={`${i}-${j}`}>
                    <span className="text-n-text-s">{p.symbol.replace('/USD', '')}</span>
                    <span className="mx-1 text-n-text">${p.price.toLocaleString('en-US', { minimumFractionDigits: p.price < 10 ? 2 : 0, maximumFractionDigits: p.price < 10 ? 2 : 0 })}</span>
                    <span className={up ? 'text-n-green' : 'text-n-red'}>{up ? '▲' : '▼'}{up ? '+' : ''}{p.changePct24h.toFixed(1)}%</span>
                    <span className="mx-3 text-n-border-b">│</span>
                  </span>
                );
              })}
            </span>
