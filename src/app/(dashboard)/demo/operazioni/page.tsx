...
export default function DemoOperazioniPage() {
  ...
  const filtered = allTrades
    ...
    .sort((a, b) => {
      ...
      if (sortField === 'date') return dir * ((a.entryAt ?? '') > (b.entryAt ?? '') ? 1 : -1);
      if (sortField === 'pnl') return dir * ((a.netPnl ?? 0) - (b.netPnl ?? 0));
  ...
  const handleExport = () => {
    ...
    a.href = url; a.download = `demo-operazioni-${new Date().toISOString().slice(0, 10)}.csv`; a.click();
  ...
  if (allTrades.length === 0) {
    return (
      <div className="space-y-5">
        <h1 className="text-xl font-bold text-n-text">Operazioni Demo</h1>
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-n-border bg-n-card/50 py-16">
          ...
          <p className="mt-1 text-xs text-n-dim">Avvia il bot dalla pagina Strategy per generare operazioni.</p>
          <Link href="/strategy" className="mt-4 rounded-lg bg-n-accent-dim px-4 py-2 text-xs font-semibold text-accent">Vai a Strategy</Link>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex rounded-lg border border-n-border">
          {(['all', 'open', 'closed'] as const).map((f) => (
            <button key={f} onClick={() => { setStatusFilter(f); setPage(0); }} className={`px-3 py-1.5 text-xs font-medium transition-colors ${statusFilter === f ? 'bg-n-accent-dim text-accent' : 'text-n-dim hover:text-n-text'}`}>
        ...
        <div className="flex rounded-lg border border-n-border">
          {(['all', 'LONG', 'SHORT'] as const).map((f) => (
            <button key={f} onClick={() => { setSideFilter(f); setPage(0); }} className={`px-3 py-1.5 text-xs font-medium transition-colors ${sideFilter === f ? 'bg-n-accent-dim text-accent' : 'text-n-dim hover:text-n-text'}`}>
              {f === 'all' ? 'Tutte' : f}
            </button>
          ))}
        </div>
      ...
      <div className="overflow-hidden rounded-xl border border-n-border bg-n-card">
              ...
              {paginated.map((t) => (
                <tr key={t.id} className="border-b border-n-border/50 transition-colors hover:bg-n-card-h">
                  ...
                  <td className="px-3 py-2.5">
                    <span className={`flex items-center gap-1 text-xs font-semibold ${t.side === 'LONG' ? 'text-n-green' : 'text-n-red'}`}>
                  ...
                  <td className="px-3 py-2.5">
                    {t.netPnl != null ? (
                      <span className={`font-mono text-xs font-semibold ${t.netPnl >= 0 ? 'text-n-green' : 'text-n-red'}`} suppressHydrationWarning>{fmtPnl(t.netPnl)}</span>
                    ) : <span className="text-xs text-n-dim">—</span>}
                  </td>
