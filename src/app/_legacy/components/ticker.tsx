...
export function Ticker() {
  ...
  return (
    <div className="h-12 overflow-hidden bg-n-bg-s border-b border-n-border/50">
      <div className="ticker-wrap h-full flex items-center">
        <div className="ticker-content whitespace-nowrap font-mono text-[15px]">
          {[0, 1].map(i => (
            <span key={i} className="inline-block">
              {prices.map((p, j) => {
                ...
                return (
                  <span key={`${i}-${j}`}>
                    <span className="font-bold text-n-text">{p.symbol.replace('/USD', '')}</span>
                    <span className="mx-2 text-n-text-s">${p.price.toLocaleString('en-US', { minimumFractionDigits: p.price < 10 ? 2 : 0, maximumFractionDigits: p.price < 10 ? 2 : 0 })}</span>
                    <span className={`font-semibold ${up ? 'text-n-green' : 'text-n-red'}`}>{up ? '▲' : '▼'}{up ? '+' : ''}{p.changePct24h.toFixed(1)}%</span>
                    <span className="mx-5 text-n-border-b">│</span>
                  </span>
                );
              })}
            </span>
